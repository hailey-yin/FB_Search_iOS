//
//  GroupsResultsViewController.swift
//  FBSearch
//
...
class GroupsResultsViewController: UIViewController, UITableViewDelegate, UITableViewDataSource  {
    ...
    @IBAction func previous(_ sender: Any) {
        if keyword != nil {
            url = URL(string:GroupsArray["paging"]["previous"].string!)
            getUsersData(url)
        } else {
    ...
    @IBAction func next(_ sender: Any) {
        if keyword != nil {
            url = URL(string:GroupsArray["paging"]["next"].string!)
            getUsersData(url)
        } else {
            currentPageInFavorite += 1
            getFavorite()
    ...
    override func viewDidLoad() {
        ...
        if keyword != nil {
            url = URL(string:"https://graph.facebook.com/v2.8/search?q="+keyword+"&type=group&fields=id,name,picture.width(700).height(700)&limit=9&access_token=EAAVKzosa82MBADOXGg6ZCboNZBuVshiJOCxpDTSb2GZC0q8wrX9V2Gfu9YwEHyX5JwRZAvScGySyLglS9ZC3LAgaLSwy0ZCyWglroZBhuNNLGP07laTeTriZAFQcdhC6YatuF7FaYSPXWnSzZArCTKZAXQbvk0kgLdOjQZD")
    ...
    func getFavorite() {
        if(UserDefaults.standard.array(forKey: "group") != nil){
            Favorite = UserDefaults.standard.array(forKey: "group")! as! [[String]]
            if keyword == nil {
        ...
        Alamofire.request(url).responseJSON { response in
            ...
            case true:
                if let value = response.result.value {
                    self.GroupsArray = JSON(value)
                    if self.GroupsArray["paging"]["previous"].string != nil {
                        self.PreviousBtn.isEnabled = true
                    } else {
                        self.PreviousBtn.isEnabled = false
                    }
                    if self.GroupsArray["paging"]["next"].string != nil {
                        self.NextBtn.isEnabled = true
                    } else {
                        self.NextBtn.isEnabled = false
                    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if keyword != nil {
            let name = GroupsArray["data"][indexPath.row]["name"].string
            cell?.name.text = name
            let url = URL(string: GroupsArray["data"][indexPath.row]["picture"]["data"]["url"].string!)
            let data = try! Data(contentsOf: url!)
            cell?.img.image = UIImage(data: data)
            cell?.favoriteBtn.setImage(UIImage(named: "empty.png"), for: .normal)
            for s in favoriteString {
                if GroupsArray["data"][indexPath.row]["id"].string == s[0] {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if keyword != nil {
            SelectedItem = GroupsArray["data"][indexPath.row]
        } else {
        ...
        performSegue(withIdentifier: "showGroupsDetails", sender: self)
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if(segue.identifier == "showGroupsDetails" && SelectedItem["id"] != nil) {
